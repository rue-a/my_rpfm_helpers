...
def enum_values_str(ev: dict) -> str:
    if not ev:
        return ""
    pairs = sorted((int(k) if isinstance(k, (int, str)) else k, v) for k, v in ev.items())
...
def render_field_row(field: dict) -> str:
    ...
    if ref:
        ...
        ref_cell  = (f'<a class="tag tag-ref ref-link" data-goto="{e(ref_table)}" href="?q={e(ref_table)}">'
                     f'{e(ref_table)}</a>'
...
def generate_html(schema: dict) -> str:
    ...
    sidebar_links = "\n".join(
        f'<a href="#{e(n)}" data-name="{e(n)}">{e(n)}</a>'
